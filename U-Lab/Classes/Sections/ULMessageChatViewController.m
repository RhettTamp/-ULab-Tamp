...
@interface ULMessageChatViewController ()
...
@property (nonatomic, strong) JMSGUser *user;  /**< 聊天对象 */
@end

@implementation ULMessageChatViewController
...
- (void)viewDidLoad
{
    ...
    NSArray *messageArray = [self.conversation messageArrayFromNewestWithOffset:nil limit:@30] ;
...
- (void)setNavigationBar
{
    ...
    [[detailButton rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(id x) {
        @strongify(self)
        ULMessageUserDetailViewController *creatVC = [[ULMessageUserDetailViewController alloc] initWithUser:self.user];
...
- (void)didPressSendButton:(UIButton *)button withMessageText:(NSString *)text senderId:(NSString *)senderId senderDisplayName:(NSString *)senderDisplayName date:(NSDate *)date
{
    ...
    {
        JSQMessage *sendMessage = [[JSQMessage alloc] initWithSenderId:senderId senderDisplayName:@"" date:[NSDate date] text:[NSString stringWithFormat:@"%@(未发送)", text]];
        [self.messages addObject:sendMessage];
        [self finishSendingMessage];
        return;
    }
    JSQMessage *sendMessage = [[JSQMessage alloc] initWithSenderId:senderId senderDisplayName:@"" date:[NSDate date] text:text];
    [self.messages addObject:sendMessage];
    [self finishSendingMessage];
}
